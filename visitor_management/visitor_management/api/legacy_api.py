...
def _parse_employee_barcode(qr_data):
	...
	try:
		...
		if isinstance(data, dict):
			employee_code = (
				data.get("employee") or data.get("employee_id") or data.get("name") or data.get("code")
			)
...
def bulk_employee_entry_action(entry_ids, action, reason=""):
	...
	if not _is_employee_entry_manager():
		frappe.throw(
			_("Hanya HR Manager, Visitor Manager, atau System Manager yang dapat melakukan bulk action")
		)
