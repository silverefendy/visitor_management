...
def _qr_data_uri(data):
    try:
        ...
        frappe.log_error(
            message=frappe.get_traceback(), title="Employee Barcode Generate Error"
        )
        return None
...
def _get_employee_from_barcode(qr_data):
    ...
    if frappe.db.exists("Employee Entry Request", employee_code):
        entry_employee = frappe.db.get_value(
            "Employee Entry Request", employee_code, "employee"
        )
        if entry_employee:
...
def get_employee_by_barcode(qr_data):
    ...
    return {
        ...
        "check_in_time": (
            str(open_entry.check_in_time)
            if open_entry and open_entry.check_in_time
            else None
        ),
        "approved_at": (
            str(open_entry.approved_at)
            if open_entry and open_entry.approved_at
            else None
        ),
        "completed_at": (
            str(open_entry.completed_at)
            if open_entry and open_entry.completed_at
            else None
        ),
    }
...
def scan_employee_entry_barcode(qr_data, action):
    ...
    if action == "checkin":
        if open_entry:
            if open_entry.status == "Completed":
                frappe.throw(
                    _(
                        "Karyawan sudah Completed. Gunakan mode Check Out untuk scan pulang."
                    )
                )
            return _employee_entry_response(
                open_entry,
                _("Pengajuan masuk sudah ada dengan status {0}.").format(
                    open_entry.status
                ),
            )
        ...
        frappe.db.commit()
        return _employee_entry_response(
            doc, _("Pengajuan check-in karyawan dibuat. Menunggu approval.")
        )
    ...
    if action == "checkout":
        ...
        if open_entry.status != "Completed":
            frappe.throw(
                _("Belum bisa check-out. Status saat ini: {0}").format(
                    open_entry.status
                )
            )
...
def bulk_employee_entry_action(entry_ids, action, reason=""):
    ...
    if not _is_employee_entry_manager():
        frappe.throw(
            _(
                "Hanya HR Manager, Visitor Manager, atau System Manager yang dapat melakukan bulk action"
            )
        )
...
def print_visitor_badge(visitor_id):
    ...
    site_name = (
        frappe.db.get_single_value("System Settings", "site_name") or "Perusahaan"
    )
    purpose = v.visit_purpose or ""
    purpose_display = (purpose[:50] + "...") if len(purpose) > 50 else purpose
    qr_img = (
        f'<img class="qr" src="{v.qr_code_image}" alt="QR">' if v.qr_code_image else ""
    )
