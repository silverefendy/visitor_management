...
function requestApi(method, args, httpMethod, ok, fail) {
	...
	for (var k in args)
		p.append(k, typeof args[k] === "object" ? JSON.stringify(args[k]) : args[k]);
...
function bindEmployeeSuggest() {
	...
	input.addEventListener("input", function () {
		...
		employeeSuggestTimer = setTimeout(function () {
			fetchEmployeeSuggestions(keyword);
		}, 200);
	});
}
...
function fetchEmployeeSuggestions(keyword) {
	...
	api(
		"visitor_management.visitor_management.api.search_employee_entry_candidates",
...
function setScanTarget(target) {
	...
	scanTarget = target;
	document.getElementById("target-visitor").className =
		"target-btn" + (target === "visitor" ? " active" : "");
	document.getElementById("target-employee").className =
		"target-btn" + (target === "employee" ? " active" : "");
	document.getElementById("scan-title").textContent =
		target === "visitor" ? "Scan QR / Barcode Tamu" : "Scan Barcode Karyawan";
	document.getElementById("vid-input").placeholder =
		target === "visitor"
			? "Ketik Visitor ID (VIS-2026-05-00001)"
			: "Ketik kode karyawan (EMP:HR-EMP-00001 / Employee ID)";
	setModeTitle();
	reset();
}
...
function setMode(m) {
	...
	document.getElementById("btn-in").className =
		"mode-btn" + (m === "checkin" ? " active-in" : "");
	document.getElementById("btn-out").className =
		"mode-btn" + (m === "checkout" ? " active-out" : "");
	setModeTitle();
	reset();
}
...
function setModeTitle() {
	...
	document.getElementById("mode-title").textContent =
		(mode === "checkin" ? "Check In " : "Check Out ") + targetLabel;
}
...
function cari() {
	...
	if (!val) {
		alert2(
			"warning",
			scanTarget === "visitor" ? "Masukkan Visitor ID" : "Masukkan kode karyawan",
		);
		return;
	}
	loadRecord(val);
...
function loadEmployee(input) {
	...
	if (!qr) {
		alert2("warning", "Data barcode / kode karyawan kosong");
		return;
	}
	...
	api(
		...
		function (emp) {
			if (!emp || emp.error) {
				alert2("error", emp ? emp.error : "Karyawan tidak ditemukan");
				return;
			}
...
function mulaiScanner() {
	...
	if (!window.isSecureContext && !["localhost", "127.0.0.1"].includes(location.hostname)) {
		alert2(
			"error",
			"Browser hanya mengizinkan kamera dari HTTPS atau localhost. Buka halaman via HTTPS untuk memakai scanner kamera.",
		);
		return;
	}
	...
	scanner.render(
		function (decodedText) {
			...
			document.getElementById("camera-status").textContent =
				"Barcode terbaca. Memuat data...";
			document.getElementById("vid-input").value =
				decodedText.charAt(0) === "{" ? "" : decodedText;
			stopScanner();
	...
	document.getElementById("camera-status").textContent =
		scanTarget === "visitor"
			? "Kamera aktif. Arahkan ke QR code visitor."
			: "Kamera aktif. Arahkan ke barcode karyawan.";
}
...
function tampilEmployee(emp) {
	...
	updateProcessButton(
		mode === "checkin" ? bisaIn : bisaOut,
		status,
		mode === "checkin" ? "AJUKAN CHECK IN KARYAWAN" : "KONFIRMASI CHECK OUT KARYAWAN",
	);
}
		...
		.map(function (v) {
			if (options.mode === "rejected") {
				return (
					...
					visitorCell(v) +
					"<td>" +
					esc(v.host_employee_name || "-") +
					"</td>" +
					"<td>" +
					timeText(v.check_in_time) +
					"</td>" +
					"<td>" +
					esc(v.rejected_reason || "-") +
					"</td>" +
					"<td>" +
					statusPill(v.status) +
					"</td>" +
			...
			return (
				...
				statusPill(v.status) +
				"</td>" +
				"<td><button onclick=\"pilih('" +
				esc(v.name) +
				"', '" +
				(options.mode === "checkout" ? "checkout" : "checkin") +
				'\')" class="btn btn-dark" style="font-size:11px;padding:4px 10px">Pilih</button></td>' +
				"</tr>"
			);
		})
		.join("");
...
function renderEmpty(id, text, error) {
	document.getElementById(id).innerHTML =
		'<p class="empty"' + (error ? ' style="color:#e74c3c"' : "") + ">" + text + "</p>";
}
...
function pilih(id, targetMode) {
	if (targetMode && targetMode !== mode) {
		mode = targetMode;
		document.getElementById("btn-in").className =
			"mode-btn" + (mode === "checkin" ? " active-in" : "");
		document.getElementById("btn-out").className =
			"mode-btn" + (mode === "checkout" ? " active-out" : "");
		setModeTitle();
	}
...
function initScannerPage() {
	bindClick("target-visitor", function () {
		setScanTarget("visitor");
	});
	bindClick("target-employee", function () {
		setScanTarget("employee");
	});
	bindClick("btn-in", function () {
		setMode("checkin");
	});
	bindClick("btn-out", function () {
		setMode("checkout");
	});
	setModeTitle();
	muatAktif();
