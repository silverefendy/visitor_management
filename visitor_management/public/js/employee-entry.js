...
function refreshCsrfToken(ok, fail) {
	fetch("/api/method/visitor_management.visitor_management.api.get_csrf_token", {
		...
		.then(function (data) {
			...
			if (!token) {
				if (fail)
					fail("CSRF token tidak ditemukan. Silakan refresh halaman lalu coba lagi.");
				return;
			}
...
function createEntry() {
	...
	api(
		"visitor_management.visitor_management.api.create_employee_entry",
		{ purpose: purpose },
		function (response) {
			showNotice(
				"success",
				response && response.message ? response.message : "Pengajuan dibuat.",
			);
...
function loadBarcode() {
	api("visitor_management.visitor_management.api.get_my_employee_barcode", {}, function (data) {
		...
		document.getElementById("barcode-wrap").innerHTML =
			(data.qr_image
				? '<img class="barcode-img" src="' + esc(data.qr_image) + '" alt="QR Karyawan">'
				: "") +
			'<div class="barcode-info">' +
			"<strong>" +
			esc(data.employee_name) +
			"</strong>" +
			"<span>Employee ID: " +
			esc(data.employee) +
			"</span>" +
...
function loadData() {
	api("visitor_management.visitor_management.api.get_employee_entry_data", {}, function (data) {
		...
		document.getElementById("manager-panel").style.display = data.is_manager
			? "block"
			: "none";
		document.getElementById("active-panel").style.display = data.is_manager ? "block" : "none";
		document.getElementById("completed-panel").style.display = data.is_manager
			? "block"
			: "none";
...
function renderTable(id, rows, selectable) {
	...
	var checkboxHeader = selectable
		? '<th><input type="checkbox" onchange="toggleAll(\'' + id + "', this.checked)\"></th>"
		: "";
	...
	var body = rows
		.map(function (row) {
			return (
				...
				"</td>" +
				"<td>" +
				esc(timeText(row.check_in_time)) +
				"</td>" +
				"<td>" +
	...
	el.innerHTML =
		"<table><thead><tr>" +
		checkboxHeader +
		"<th>Karyawan</th><th>Departemen</th><th>Keperluan</th><th>Check In</th><th>Status</th>" +
		actionHeader +
		"</tr></thead><tbody>" +
		body +
		"</tbody></table>";
...
function rowActions(listId, id) {
	...
	if (listId === "pending-list") {
		return (
			'<button class="btn btn-success btn-sm" onclick="entryAction(\'' +
			safeId +
			"', 'approve')\">Approve</button> " +
			'<button class="btn btn-danger btn-sm" onclick="rejectEntry(\'' +
			safeId +
			"')\">Reject</button>"
		);
	}
	if (listId === "active-list") {
		return (
			'<button class="btn btn-primary btn-sm" onclick="entryAction(\'' +
			safeId +
...
function toggleAll(id, checked) {
	Array.prototype.forEach.call(
		document.querySelectorAll("#" + id + " .row-check"),
		function (el) {
			el.checked = checked;
		},
	);
...
function selectedIds(id) {
	return Array.prototype.map.call(
		document.querySelectorAll("#" + id + " .row-check:checked"),
		function (el) {
			return el.value;
		},
	);
...
function bulkAction(action, listId, reason) {
	...
	api(
		"visitor_management.visitor_management.api.bulk_employee_entry_action",
		{
		...
		function (response) {
			...
			if (errors.length) {
				...
				showNotice(
					"success",
					"Bulk action selesai: " + results.length + " data diproses.",
				);
			}
			loadData();
		},
...
function entryAction(id, action, reason) {
	api(
		"visitor_management.visitor_management.api.employee_entry_action",
		{
			entry_id: id,
			action: action,
			reason: reason || "",
		...
		function (response) {
			showNotice(
				"success",
				response && response.message ? response.message : "Aksi berhasil diproses.",
			);
			loadData();
		},
	);
