...
function card(v, type) {
	var approveActions =
		type === "pending"
			? '<button class="btn btn-success" onclick="approveVisitor(\'' +
				esc(v.name) +
				"')\">Approve</button>" +
				'<button class="btn btn-danger" onclick="rejectVisitor(\'' +
				esc(v.name) +
				"')\">Reject</button>"
			: '<button class="btn btn-primary" onclick="completeVisit(\'' +
				esc(v.name) +
				"')\">Selesai Kunjungan</button>";
	...
	return (
		...
		'<div class="field purpose"><label>Keperluan</label><span>' +
		esc(v.visit_purpose) +
		"</span></div>" +
		"</div>" +
		'<div class="actions">' +
...
function field(label, value) {
	return (
		'<div class="field"><label>' + esc(label) + "</label><span>" + esc(value) + "</span></div>"
	);
}
...
function approveVisitor(id) {
	api(
		"visitor_management.visitor_management.api.approve_visitor",
		{ visitor_id: id },
		function (r) {
...
function rejectVisitor(id) {
	...
	if (!reason) return;
	api(
		"visitor_management.visitor_management.api.reject_visitor",
		{ visitor_id: id, reason: reason },
		function (r) {
...
function completeVisit(id) {
	...
	api(
		"visitor_management.visitor_management.api.complete_visit",
		{ visitor_id: id },
		function (r) {
